...
if(interactive()){
  .args <-  c('/Users/hamishgibbs/Documents/Covid-19/facebook_mobility_uk/data/interim/mobility_days.csv',
              ...
              '/Users/hamishgibbs/Documents/Covid-19/facebook_mobility_uk/reports/figures/tile_pop_eth_comparison.png')
} else {
...
p <- inter_mob %>% 
  left_join(qk_eth, by = c('quadkey' = 'quadkey_12')) %>% 
  ggplot() + 
  geom_point(aes(x = pop_ratio * 100, y = 100* (1 - wm_perc_white), color = country), size = 0.1) +
  scale_color_discrete_qualitative('Harmonic') + 
  facet_wrap(~country, nrow = 2, scales = 'free') + 
  theme_bw() + 
  xlab('% Facebook users') +
  ylab('Tile Mean % Ethnic Minority') +
  plot_default_theme + 
